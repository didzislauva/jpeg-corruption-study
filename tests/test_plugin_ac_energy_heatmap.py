...
def test_ac_energy_heatmap_plugin_runs(tmp_path: Path, monkeypatch) -> None:
    clear_registry_for_tests()
    load_plugins(force=True)
    plugin = get_plugin("ac_energy_heatmap")
    assert plugin is not None

    calls = {}

    def fake_write_ac_energy_heatmap(
        input_path: str,
        out_path: str,
        debug: bool,
        cmap: str = "magma",
    ...
    mod = importlib.import_module("jpeg_fault.core.plugins.ac_energy_heatmap.plugin")
    monkeypatch.setattr(mod, "write_ac_energy_heatmap", fake_write_ac_energy_heatmap)
    ...
    ctx = AnalysisContext(
        ...
        debug=True,
        params={
            "out_path": str(tmp_path / "custom_ac.png"),
            "cmap": "viridis",
            "plane_mode": "green",
            "block_size": 16,
        },
    ...
    assert result.plugin_id == "ac_energy_heatmap"
    assert result.outputs == [str(tmp_path / "custom_ac.png")]
...
def test_ac_energy_heatmap_plugin_uses_descriptive_default_name(tmp_path: Path, monkeypatch) -> None:
    clear_registry_for_tests()
    load_plugins(force=True)
    plugin = get_plugin("ac_energy_heatmap")
    assert plugin is not None
    ...
    expected = tmp_path / "sample_ac_energy_heatmap_bt709_b16.png"
