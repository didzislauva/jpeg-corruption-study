...
from .._shared.dct_heatmap import write_ac_energy_heatmap
from ...tui_plugin_registry import register_tui_plugin
from ...tui_plugin_types import TuiPluginSpec


@dataclass(frozen=True)
class AcEnergyHeatmapPlugin(AnalysisPlugin):
    id: str = "ac_energy_heatmap"
    label: str = "AC Energy Heatmap"
    supported_formats: set[str] = frozenset({"jpeg"})
    requires_mutations: bool = False
    needs: frozenset[str] = frozenset()
    params_spec: tuple[PluginParamSpec, ...] = (
        PluginParamSpec(name="out_path", label="Output path", type="path", help="Optional output image path for the generated AC energy heatmap."),
        PluginParamSpec(name="cmap", label="Colormap", type="string", default="magma", help="Matplotlib colormap name for the rendered heatmap."),
        PluginParamSpec(
            name="plane_mode",
            label="Plane mode",
            type="choice",
    ...
    def run(self, input_path: str, context: AnalysisContext) -> AnalysisResult:
        ...
        cmap = str(params.get("cmap", "magma"))
        plane_mode = str(params.get("plane_mode", "bt601"))
        block_size = int(params.get("block_size", 8))
        default_name = f"{Path(input_path).stem}_ac_energy_heatmap_{plane_mode}_b{block_size}.png"
        ...
        block_rows, block_cols = write_ac_energy_heatmap(
            input_path,
            str(out_path),
            context.debug,
            cmap=cmap,
...
plugin = AcEnergyHeatmapPlugin()
register(plugin)

register_tui_plugin(
    TuiPluginSpec(
        id="ac_energy_heatmap",
        label="AC Energy Heatmap",
        panel_id="graphic-output",
        panel_label="Graphic Output",
        tab_label="AC Energy",
        analysis_plugin_id="ac_energy_heatmap",
    )
)
